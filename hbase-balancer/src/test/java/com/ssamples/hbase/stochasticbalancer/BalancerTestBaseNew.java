 ...
 *
 */
public class BalancerTestBaseNew {
  private static final Logger LOG = LoggerFactory.getLogger(BalancerTestBaseNew.class);
  static int regionId = 0;
  protected static Configuration conf;
  protected static StochasticLoadBalancerNew loadBalancer;
  ...
  public static void beforeAllTests() throws Exception {
    ...
    conf.setBoolean("hbase.master.balance.bytable", true);
    LOG.info("Creating loadBalancer and setting conf table level");
    loadBalancer = new StochasticLoadBalancerNew();
  ...
  protected void testWithCluster(Map<ServerName, List<RegionInfo>> serverMap,
    ...
    for (Map.Entry<ServerName, List<RegionInfo>> rInfo : serverMap.entrySet()) {
      ...
      while (rI.hasNext()) {
    	  ...
    	  cp = rand.nextInt(1000);
    	  memSt = rand.nextInt(10000);
    	  stFile = rand.nextInt(10000);
    	  String regionName = rI.next().getRegionNameAsString();
    	  Deque<BalancerRegionLoad> rLoadDeque = new  ArrayDeque<BalancerRegionLoad>();
    	  rLoadDeque.add(new BalancerRegionLoad(new BalancerTestMetrics(reads,writes,cp,memSt,stFile)));
    	  reads = rand.nextInt(10000);
    	  writes = rand.nextInt(10000);
    	  cp = rand.nextInt(1000);
    	  memSt = rand.nextInt(10000);
    ...
    if (assertFullyBalanced || assertFullyBalancedForReplicas) {
      ...
      if (assertFullyBalanced) {
        ...
        List<RegionPlan> secondPlans =  loadBalancer.balanceCluster(serverMap, serverWeights);
	...
	protected void testWithClusterTables(int numNodes, int numRegions, 
			int numRegionsPerServer, int replication,
			int numTables, 
